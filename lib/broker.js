 ...
 * @param {Number} connectionInfo.port - port (default 5672)
    ...
    value: function () {
      ...
      if (self._connection) {
        ...
        if (!self._connectionInfo.hostname) {
          ...
          connectionStr = self._connectionInfo.hostname + self._connectionInfo.vhost;
        }
        ...
        var auth = credentialsFromUrl(parsedUrl);
        var credentials = self._amqp.credentials.plain(self._connectionInfo.username || auth.username, self._connectionInfo.password || auth.password);
