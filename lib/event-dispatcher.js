  ...
  function dispatch(eventNames) {
    ...
    return events.reduce((promise, eventName) => {
      return promise.then((result) => {
        var handlers = _.filter(buckets, function(item){
        ...
        return Promise.try(function(){
          return Promise.all(_.map(handlers, handler => handler.handler.apply(null, [eventName].concat(args))));
        }).then(function(){
